...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      try {
        // Tentar obter um novo token (se você tiver um endpoint de refresh token)
        // const refreshToken = await SecureStore.getItemAsync('refreshToken');
        // const response = await axios.post('http://192.168.1.100:3000/auth/refresh', { refreshToken });
        ...
        // Se não tiver refresh token, limpar dados de autenticação
